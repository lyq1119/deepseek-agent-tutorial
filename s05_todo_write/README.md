...
> **DeepSeek implementation**: `code.py` uses `client.chat.completions.create(...)`, reads `message.tool_calls`, and sends tool results as `{"role": "tool", "tool_call_id": ...}`. The `block` mentioned below is the small adapter passed to existing hooks.
